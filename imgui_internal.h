...
namespace ImGui
{
    ...
    inline ImDrawList*      GetForegroundDrawList(ImGuiContext* ctx, ImGuiWindow* window) { return GetForegroundDrawList(ctx, window->Viewport); }
    ...
    IMGUI_API void          StartMouseMovingWindowOrNode(ImGuiContext* ctx, ImGuiWindow* window, ImGuiDockNode* node, bool undock);
    ...
    // Viewports
    IMGUI_API void          TranslateWindowsInViewport(ImGuiContext* ctx, ImGuiViewportP* viewport, const ImVec2& old_pos, const ImVec2& new_pos);
    IMGUI_API void          ScaleWindowsInViewport(ImGuiContext* ctx, ImGuiViewportP* viewport, float scale);
    IMGUI_API void          DestroyPlatformWindow(ImGuiContext* ctx, ImGuiViewportP* viewport);
    IMGUI_API void          SetWindowViewport(ImGuiWindow* window, ImGuiViewportP* viewport);
    IMGUI_API void          SetCurrentViewport(ImGuiContext* ctx, ImGuiWindow* window, ImGuiViewportP* viewport);
    IMGUI_API const ImGuiPlatformMonitor*   GetViewportPlatformMonitor(ImGuiContext* ctx, ImGuiViewport* viewport);
    IMGUI_API ImGuiViewportP*               FindHoveredViewportFromPlatformWindowStack(ImGuiContext* ctx, const ImVec2& mouse_platform_pos);
...
//#endif
    ...
    IMGUI_API const char*   GetKeyChordName(ImGuiContext* ctx, ImGuiKeyChord key_chord, char* out_buf, int out_buf_size);
    ...
    IMGUI_API bool          DockContextCalcDropPosForDocking(ImGuiContext* ctx, ImGuiWindow* target, ImGuiDockNode* target_node, ImGuiWindow* payload_window, ImGuiDockNode* payload_node, ImGuiDir split_dir, bool split_outer, ImVec2* out_pos);
    ...
    IMGUI_API bool          DockNodeBeginAmendTabBar(ImGuiContext* ctx, ImGuiDockNode* node);
    IMGUI_API void          DockNodeEndAmendTabBar(ImGuiContext* ctx);
    ...
    inline ImGuiDockNode*   GetWindowDockNode(ImGuiContext* ctx)                                      { ImGuiContext& g = *ctx; return g.CurrentWindow->DockNode; }
    IMGUI_API bool          GetWindowAlwaysWantOwnTabBar(ImGuiContext* ctx, ImGuiWindow* window);
    IMGUI_API void          BeginDocked(ImGuiContext* ctx, ImGuiWindow* window, bool* p_open);
    IMGUI_API void          BeginDockableDragDropSource(ImGuiContext* ctx, ImGuiWindow* window);
    IMGUI_API void          BeginDockableDragDropTarget(ImGuiContext* ctx, ImGuiWindow* window);
    IMGUI_API void          SetWindowDock(ImGuiContext* ctx, ImGuiWindow* window, ImGuiID dock_id, ImGuiCond cond);
    ...
    inline ImGuiDockNode*   DockBuilderGetCentralNode(ImGuiContext* ctx, ImGuiID node_id)              { ImGuiDockNode* node = DockBuilderGetNode(ctx, node_id); if (!node) return NULL; return DockNodeGetRootNode(node)->CentralNode; }
    IMGUI_API ImGuiID       DockBuilderAddNode(ImGuiContext* ctx, ImGuiID node_id = 0, ImGuiDockNodeFlags flags = 0);
    IMGUI_API void          DockBuilderRemoveNode(ImGuiContext* ctx, ImGuiID node_id);                 // Remove node and all its child, undock all windows
    IMGUI_API void          DockBuilderRemoveNodeDockedWindows(ImGuiContext* ctx, ImGuiID node_id, bool clear_settings_refs = true);
    IMGUI_API void          DockBuilderRemoveNodeChildNodes(ImGuiContext* ctx, ImGuiID node_id);       // Remove all split/hierarchy. All remaining docked windows will be re-docked to the remaining root node (node_id).
    IMGUI_API void          DockBuilderSetNodePos(ImGuiContext* ctx, ImGuiID node_id, ImVec2 pos);
    IMGUI_API void          DockBuilderSetNodeSize(ImGuiContext* ctx, ImGuiID node_id, ImVec2 size);
    IMGUI_API ImGuiID       DockBuilderSplitNode(ImGuiContext* ctx, ImGuiID node_id, ImGuiDir split_dir, float size_ratio_for_node_at_dir, ImGuiID* out_id_at_dir, ImGuiID* out_id_at_opposite_dir); // Create 2 child nodes in this parent node.
    ...
    IMGUI_API void          TabBarAddTab(ImGuiContext* ctx, ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, ImGuiWindow* window);
    ...
    IMGUI_API ImVec2        TabItemCalcSize(ImGuiContext* ctx, ImGuiWindow* window);
    ...
    IMGUI_API bool          CollapseButton(ImGuiContext* ctx, ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node);
    ...
    IMGUI_API void          DebugBreakClearData(ImGuiContext* ctx);
    IMGUI_API bool          DebugBreakButton(ImGuiContext* ctx, const char* label, const char* description_of_location);
    IMGUI_API void          DebugBreakButtonTooltip(ImGuiContext* ctx, bool keyboard_only, const char* description_of_location);
    ...
    IMGUI_API void          DebugNodeDockNode(ImGuiContext* ctx, ImGuiDockNode* node, const char* label);
